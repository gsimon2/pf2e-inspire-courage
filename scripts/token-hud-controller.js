const tokenButtonHandler = async (event, actor, token) => {
    ...
    if (inspireCourageEffects.length) {
        ...
        actor.deleteEmbeddedDocuments('Item', inspireCourageEffects.map(i => i.id));
    } else {
        btn.addClass('active');
        const effects = await actor.createEmbeddedDocuments('Item', [inspireCourageItem.data]);
        const inspireCourageEffect = effects.find(e => e.data.name === inspireCourageItem.data.name);
        const update = {_id: inspireCourageEffect._id, data: {expired: false, duration: {unit: 'unlimited'}}};
...
const createButton = (actor) => {
    ...
    getInspireCourageEffectsFromActor(actor).length ? btn.classList.add('active') : {};
